...
func (r resize) Json() string {
	return fmt.Sprintf("{\"path\":\"%s\",\"target\":\"%s\",\"height\":%d,\"width\":%d,\"type\":\"resize\"}",
		r.originPath,
		r.targetPath,
		r.height,
		r.width)
...
func (p print) Json() string {
	return fmt.Sprintf("{\"message\":\"%s\",\"type\":\"print\"}", p.message)
}
