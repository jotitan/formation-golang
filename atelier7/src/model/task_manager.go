...
func (m *Manager) GetWithStatus(id int) (Task, TaskStatus) {
...
func (m *Manager) UpdateStatus(id int, status TaskStatus) {
	task, exist := m.tasks[id]
	if exist {
		task.Status = status
	}
...
func (m *Manager) Add(task Task) Task {
	m.tasks[task.Id()] = &RunningTask{Task: task, Status: TaskPending}
	return task
}
